...
在 CommonJS 时代，模块之间通过 require 来建立联系，这种方式，使用非常的灵活，但是也带来一个问题，过于灵活导致无法对引用的模块进行分析，假如引用的文件中有大量用不到的代码，我们也清除不掉，只能照单全收，造成打包之后的代码体积大，拖慢代码的执行速度。
...
问题来了，Tree Shaking 和 sideEffects 都失效的情况下，我们怎么清除这段无用的代码？
